...
int run_cracker (hashcat_ctx_t *hashcat_ctx, hc_device_param_t *device_param, const u32 pws_cnt)
{
  ...
  for (u32 salt_pos = 0; salt_pos < hashes->salts_cnt; salt_pos++)
  {
    ...
    #ifdef ALEXDEBUG
      //kernel loops decides how many rules each time
      printf("Kernel_loops: %d\n", device_param->kernel_loops); //5,2,1 for 5 rules. 21, 10, 1 for 21 rules. Called once
    #endif
    ...
    pws_cnt = words in wordlist -- remove too long --> at most 4096

    HC is indeed a batched!
